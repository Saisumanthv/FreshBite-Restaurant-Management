...
import { BarChart3, TrendingUp, Clock, CheckCircle, Flame, AlertTriangle, RefreshCw, Plus, X, Trash2, Image as ImageIcon, LogOut, Eye, EyeOff, Key, Pencil, RotateCcw, History, ChevronDown, ChevronUp, Users } from 'lucide-react';
            ...
            <div className="flex gap-0.5 bg-gray-100 rounded-xl p-1">
              {(['dashboard', 'menu', 'staff', 'history'] as const).map(t => (
                <button key={t} onClick={() => setTab(t)}
                  className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-all capitalize ${tab === t ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
                  {t === 'history' ? (
                    <span className="flex items-center gap-1"><History size={13} /><span className="hidden sm:inline">History</span></span>
                  ) : t}
                </button>
              ))}
            </div>
